...
export async function myBalance(req: Request, res: Response) {
  try {
    const user_balance: any = await jwt.verify(req.cookies?.nkoe, SECRET_TOKEN);
...
export async function cashInOut(req: Request, res: Response) {
  try {
    const cashout: any = await jwt.verify(req.cookies?.nkoe, SECRET_TOKEN);
...
export async function seeTransactions(req: Request, res: Response) {
  const token: any = await jwt.verify(req.cookies?.nkoe, SECRET_TOKEN);
